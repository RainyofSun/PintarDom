...
public class APPCocoaLog: NSObject {
    
    //单例
    public static let shared = APPCocoaLog()
    ...
    public func registe(with env: EnvType) {
        ...
        let fileLoggerManagerDefault = APPCocoaLogFile.init(logsDirectory: logDirectory)
        /// file log
...
extension APPCocoaLog: DDLogFormatter {
...
extension NSObject {
    public func deallocPrint() {
        APPCocoaLog.debug("deinit: \(NSStringFromClass(type(of: self)))")
    }
}
